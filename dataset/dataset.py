...
class GeneSequenceDataset(Dataset):
    def __init__(self, file_path, tokenizer, max_seq_length, is_dna=True):
        ...
        if not is_dna:
            sequences = [[seq.replace('T', 'U') for seq in row] for row in sequences]
            self.features = convert_examples_to_features(sequences, speed, list_gene_all, dict_seq_nt, dict_seq_aa, dict_pro_feature, dict_ss, tokenizer, max_seq_length, cls_token='<cls>', sep_token='<sep>')
        else:
